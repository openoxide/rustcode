...
This means every provider ID in the opencode models index is recognized by `rustcode` provider initialization. None fail with unknown/unsupported initialization paths.

## Interpretation

- `needs_api_key` means endpoint and protocol are resolved and runtime is blocked only on credential availability.
- `needs_base_url` means a provider-specific endpoint cannot be inferred and must be configured via `[llm].base_url` (or `--llm-base-url`).
