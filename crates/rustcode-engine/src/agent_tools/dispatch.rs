...
use super::{ensure_allowed_keys, opt_str, opt_str_list, opt_u32, opt_u64, AgentToolRegistry};

pub(super) async fn execute_tool<'a>(
    engine: &'a Engine,
    name: &'a str,
    args: Value,
    ...
    match name {
        ...
        "read" => {
            ...
            let path = opt_str(&args, "path")?
                .ok_or_else(|| ExecutionError::Dispatch("read tool requires path".to_string()))?;
        ...
        "write" => {
            ...
            let path = opt_str(&args, "path")?
                .ok_or_else(|| ExecutionError::Dispatch("write tool requires path".to_string()))?;
        ...
        "edit" => {
            ...
            let path = opt_str(&args, "path")?
                .ok_or_else(|| ExecutionError::Dispatch("edit tool requires path".to_string()))?;
            let from = opt_str(&args, "from")?
                .ok_or_else(|| ExecutionError::Dispatch("edit tool requires from".to_string()))?;
            if from.is_empty() {
                return Err(ExecutionError::Dispatch(
        ...
        "multiedit" => {
            ...
            let path = opt_str(&args, "path")?
                .ok_or_else(|| ExecutionError::Dispatch("multiedit requires path".to_string()))?;
            let edits_arr = args.get("edits").and_then(Value::as_array).ok_or_else(|| {
            ...
            for (idx, edit) in edits_arr.iter().enumerate() {
                let old = edit
                    ...
                    .ok_or_else(|| {
                        ExecutionError::Dispatch(format!("edits[{idx}].old_string required"))
                    })?
                    .to_string();
                let new = edit
                    .get("new_string")
                    .and_then(Value::as_str)
                    .ok_or_else(|| {
                        ExecutionError::Dispatch(format!("edits[{idx}].new_string required"))
                    })?
                    .to_string();
        ...
        "batch" => execute_batch(engine, args, context, options, state).await,
        "snapshot_list" => {
        ...
        "skill" => {
            ...
            let name = opt_str(&args, "name")?
                .ok_or_else(|| ExecutionError::Dispatch("skill tool requires name".to_string()))?;
            engine.agent_tool_skill(name).await
        }
        "task" => {
            ensure_allowed_keys(&args, &["prompt", "max_steps"])?;
            let prompt = opt_str(&args, "prompt")?
                .ok_or_else(|| ExecutionError::Dispatch("task tool requires prompt".to_string()))?;
        ...
        "codesearch" => {
            ...
            let query = opt_str(&args, "query")?
                .ok_or_else(|| ExecutionError::Dispatch("codesearch requires query".to_string()))?;
        ...
        "websearch" => {
            ...
            let query = opt_str(&args, "query")?
                .ok_or_else(|| ExecutionError::Dispatch("websearch requires query".to_string()))?;
    ...
    for handle in handles {
        match handle {
            ...
            Ok((tool_name, tool_params, eng, ctx, opts, st)) => {
                ...
                let result = AgentToolRegistry::execute(
                    eng,
                    &tool_name,
                    tool_params,
                    ctx,
...
async fn execute_question(engine: &Engine, args: Value) -> Result<String, ExecutionError> {
    ...
    for q in questions_arr {
        let question = q
            ...
            .and_then(Value::as_str)
            .ok_or_else(|| {
                ExecutionError::Dispatch("each question requires a 'question' field".to_string())
            })?
            .to_string();
        let options = q
            .get("options")
...
async fn execute_plan(engine: &Engine, args: Value) -> Result<String, ExecutionError> {
    ...
    for s in steps_arr {
        let description = s
            ...
            .and_then(Value::as_str)
            .ok_or_else(|| {
                ExecutionError::Dispatch("each step requires 'description'".to_string())
            })?
            .to_string();
        let status = s
            .get("status")
...
async fn execute_todowrite(args: Value) -> Result<String, ExecutionError> {
    ...
    let todos = args
        .get("todos")
        .ok_or_else(|| ExecutionError::Dispatch("todowrite tool requires todos".to_string()))?;
    let list = todos
        .as_array()
        .ok_or_else(|| ExecutionError::Dispatch("todos must be an array".to_string()))?;

    let mut normalized = Vec::with_capacity(list.len());
    for (idx, item) in list.iter().enumerate() {
        let obj = item
            .as_object()
            .ok_or_else(|| ExecutionError::Dispatch(format!("todos[{idx}] must be an object")))?;
        for key in obj.keys() {
        ...
        let content = obj
            ...
            .and_then(Value::as_str)
            .ok_or_else(|| {
                ExecutionError::Dispatch(format!("todos[{idx}].content must be a string"))
            })?
            .trim()
            .to_string();
        if content.is_empty() {
        ...
        let status = obj
            ...
            .and_then(Value::as_str)
            .ok_or_else(|| {
                ExecutionError::Dispatch(format!("todos[{idx}].status must be a string"))
            })?
            .trim()
            .to_ascii_lowercase();
        if !matches!(
        ...
        let priority = obj
            ...
            .and_then(Value::as_str)
            .ok_or_else(|| {
                ExecutionError::Dispatch(format!("todos[{idx}].priority must be a string"))
            })?
            .trim()
            .to_ascii_lowercase();
